...
const TrendDetails = ({ article }: TrendDetailsProps) => {
  const [otherArticles, setOtherArticles] = useState<TrendArticle[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchArticles = fetch('/api/trends')
      .then((res) => res.json())
      .then((data) => {
        // Sort articles by createdAt (desc) and limit to 5
        const sortedArticles = data.sort((a: TrendArticle, b: TrendArticle) => {
          return (
            new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
          );
        });
        ...
        const filteredArticles = sortedArticles.filter(
          (article: TrendArticle) => article.id !== id
        );
    ...
    Promise.all([fetchArticles]).catch((err) =>
  ...
  const articleUrl = `https://kabayankonek.com/trends/${article.id}`;
