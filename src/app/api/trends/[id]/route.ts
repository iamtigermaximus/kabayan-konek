...
export async function DELETE(req: NextRequest) {
  ...
  try {
    await prisma.trendArticle.delete({
      where: { id },
    });
    ...
    return NextResponse.json({ message: 'Trend article deleted successfully' });
  } catch (error) {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const updatedArticle = await prisma.trendArticle.update({
      where: { id },
      data: {
        title,
        content,
